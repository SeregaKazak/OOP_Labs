...
public:
    ...
    Character(const std::string& n, int h, int a, int d, int mh = 100) // добавлен mh
        : name(n), health(h), attack(a), defense(d), maxHealth(mh) {
    } // добавлен maxHealth
...
// Метод для получения уровня здоровья
    ...
    void attackEnemy(Character& enemy) {
        ...
        if (damage > 0) {
            enemy.takeDamage(damage); // Используем takeDamage
...
int main() {
    ...
    // Монстр получает много урона
    monster.takeDamage(100);
    monster.displayInfo(); // Должно быть HP: 0
